...
class QuestionSetsRouteTest {
    ...
    fun testCreateUpdateGetGetListAndDelete() = runTest {
        ...
        var questionSetFromList = questionSetsRoutes.list(auth.accessToken, 0, Int.MAX_VALUE, SortOrder.ASC)
            .find { it.id == created.id }!!
        var questionSet = questionSetsRoutes.get(auth.accessToken, created.id)
        ...
        val updated = questionSetsRoutes.update(auth.accessToken, questionSet.id, updateRequest)
        ...
        questionSetFromList = questionSetsRoutes.list(auth.accessToken, 0, Int.MAX_VALUE, SortOrder.ASC)
            .find { it.id == updated.id }!!
        questionSet = questionSetsRoutes.get(auth.accessToken, updated.id)
        assertThat(listOf(questionSetFromList, questionSet))
            .allSatisfy {
                assertThat(it)
                    .usingRecursiveComparison()
